  ...
  async computeDimensionVeto(teacherId: string, academicYear: string) {
    ...
    if (veto.hasDimVeto) {
      const exists = await this.prisma.preConditionFlag.findFirst({
        where: { teacherId, academicYear, flagType: FlagType.DIM_VETO },
      });
      if (!exists) {
        await this.prisma.preConditionFlag.create({
          data: {
            teacherId,
            academicYear,
            flagType: FlagType.DIM_VETO,
            dimViolation: veto.vetoDimensions[0],
            gradeRestriction: GradeRestriction.NO_A_OR_ABOVE,
            confirmedById: 'SYSTEM',
            isAutoGenerated: true,
            evidence: `维度 ${veto.vetoDimensions.join(',')} 加权得分率低于 ${config.dimensionVetoThreshold}`,
          },
        });
      }
    }
