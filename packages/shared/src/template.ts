 ...
 *  2. 各维度满分与系统配置一致（默认 20/25/20/20/15）；
 *  3. 每个维度下题目分值之和 == 维度满分（test_plan U-CALC-05 的数据前置）；
 *  4. 5 维度满分合计 == 100。
 *
...
export function validateTemplate(
  ...
  for (const d of dimensions) {
    const expectMax = config.dimensionMaxScores[d.dimensionNo as DimensionNo];
    // 2. 维度满分一致
    if (d.maxScore !== expectMax) {
      errors.push({
        dimensionNo: d.dimensionNo,
        message: `维度 ${d.dimensionNo} 满分应为 ${expectMax}，当前为 ${d.maxScore}`,
      });
    }
    // 3. 题目分值之和 == 维度满分
